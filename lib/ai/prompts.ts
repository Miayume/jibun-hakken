...
aptitudeRankingの良い例:
{
  "rank": 1,
  "type": "クリエイタータイプ",
  "reason": "「作る」「新しくする」「工夫する」を選んでいること、さらに没頭体験でも「自分のクリエイティビティを外に表現できた時にすっきりする」と答えていることから、自分の内側にあるものを形にすることで充実感を得るタイプだと読み取れる",
  "insight": "デザイン・映像・ライティング・プロダクト開発など、0→1でものを作る仕事や、既存のものを自分なりに改良・表現できる環境で特に力を発揮する可能性が高いです"
}

aptitudeRankingの悪い例（全ての言葉を羅列するだけ）:
{
  "rank": 1,
  "type": "リーダータイプ",
  "reason": "「挑戦する」「達成する」「実行する」「教える」「助ける」「支える」「育てる」「導く」「伝える」「つなぐ」「守る」「応援する」「調整する」といった言葉を選んでいるため"
}
...
次のJSON形式のみで出力してください。各項目は配列で、配列の要素は { "point": "...", "reason": "...", "insight": "..." } の形にしてください。各項目名の意味は日本語コメントの通りです:
{
  ...
  //
  // タイプの定義（以下9つから選ぶ）:
  // ・経営者タイプ: 組織全体を俯瞰し、目標設定・意思決定・リソース管理で力を発揮する
  // ・リーダータイプ: チームを鼓舞し、人を動かして方向性を示す。先頭に立って引っ張る
  // ・起業家タイプ: 新しいビジネスや価値を生み出す。リスクを取って挑戦し続ける
  // ・クリエイタータイプ: 独自のアイデアや表現で新しいものを作り出す。感性・創造性が強み
  // ・研究者タイプ: 深く掘り下げて考え、分析・探求・体系化することに喜びを感じる
  // ・教育者タイプ: 人の成長を支え、知識やスキルを分かりやすく伝えることが得意
  // ・コミュニケータータイプ: 人とつながり、伝え、橋渡しすることに長けている
  // ・サポータータイプ: 人を支え、助けることに意義を感じる。縁の下の力持ちとして活躍
  // ・プロジェクトマネージャータイプ: 計画・調整・進行管理で物事を確実に形にする
  //
  // 【重要】以下の全データを総合してタイプを判断すること:
  // - 選んだ言葉のパターン（どの言葉の組み合わせが際立つか）
  // - ジャーナルの記録（何にワクワクし、何にストレスを感じてきたか）
  // - 没頭体験の深掘り（why1〜why3で語られた本質的な動機・価値観）
  // - 最高の自分のビジョン（どんな状況・環境・人間関係を理想としているか）
  //
  // 分析の書き方:
  // - reason: 言葉の選択・ジャーナル・没頭体験・ビジョンの中から、このタイプを最も強く裏付ける具体的な証拠を2〜3点挙げ、なぜこのタイプなのかを1〜2文で説明する。証拠は「〇〇という記録」「〇〇と答えていること」のように具体的に示す
  // - insight: このタイプが最も輝く具体的な仕事・役割・環境を「〜の可能性が高いです」「〜に向いている傾向があります」の表現で1〜2文で述べる
}`;
}
