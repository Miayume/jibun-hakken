...
次のJSON形式のみで出力してください。各項目は配列で、配列の要素は { "point": "...", "reason": "...", "insight": "..." } の形にしてください。各項目名の意味は日本語コメントの通りです:
{
  ...
  //
  // 分析の方法:
  // 1. 選んだ言葉の「パターンと組み合わせ」からタイプを判断する（全ての言葉を並べない）
  // 2. reason: このタイプを最も強く示す2〜3語に絞り、なぜそのパターンがこのタイプを示すのかを1〜2文で具体的に説明する。ジャーナルの記録や没頭体験も活用してよい
  // 3. insight: このタイプが最も輝く場面・仕事・環境を「〜の可能性が高いです」「〜に向いている傾向があります」のような表現で1〜2文で述べる。具体的な職種・役割・働き方のイメージを含める
}`;
}
